...
Judge). Stage D adds a 13th, `rubric_judge`, for automated scoring.
...
| **D — Evaluation** | Writes one report per behaviour to `human_review/reports/`, optionally auto-scored by `rubric_judge`. | `main.py --stage-d` |
...
| `run_eval_matrix.py` | Repo-root runner: Stage D across a size ladder / tier of target models (≤7B) from `eval/model_matrix.py`, marked `is_part_of_scaling_sweep=True`. **Strictly sequential** — one model downloaded, run, freed, and (with `--purge-downloads`) deleted from the HF cache before the next. Resumable (`--skip-done`), `--check` for a fast wiring-only pass, one failing model doesn't sink the sweep. For handing to a teammate with compute. |
...
| `skeptic.py` / `judge.py` / `rubric_judge.py` | Falsification, verdict, and the Stage-D automated scorer. |
...
## 9. How a human reviewer should review it

Three independent expert scorers, 1–5 per dimension, no discussion until all
three submit. Source of truth: `automechinterp/eval/rubrics.py`.

### 9.1 The process

1. Reports are in `human_review/reports/` — one `.md` per (model, behaviour).
   Generated by `python3 main.py --stage-d`.
2. Copy `human_review/scoring_template.csv` to
   `human_review/scores/<your_name>.csv` — do not edit in place.
3. Score every applicable rubric 1–5. Leave inapplicable cells blank
   (angle-specific rubrics only apply to that angle; each report's
   `applicable_rubrics` list is in the Stage-D JSON).
4. When all three are in: `python3 human_review/aggregate_scores.py` combines
   them and computes inter-rater agreement.

### 9.2 Core rubric dimensions (1–5, score on every report)

| # | Dimension | What to check |
|---|---|---|
| 1 | **Localization accuracy** | Did it identify the truly load-bearing layer(s)? Check against the *Skeptic's ablation evidence*, not the report's own claim. |
| 2 | **Circuit precision / recall** | Of the heads claimed, how many are real vs padding? How many real ones were missed? |
| 3 | **Verification rigour** | Did the claim actually survive all four Skeptic checks, or is it asserted? A partial / empty evidence transcript scores low *regardless of the verdict line*. |
| 4 | **Explanatory depth** | Does it explain the *mechanism* (why this head does this), not just report a coordinate? |
| 5 | **Faithfulness / evidence-grounding** | Is every claim traceable to an actual tool-call result in the report? No unsupported leaps. |
| 6 | **Clarity & structure** | Organised, readable, uses tables where they help. |
| 7 | **Calibration** | Does the verdict (Confirmed / Probable / Speculative / Refuted) match the evidence? A confident claim the evidence doesn't support scores low even if it sounds plausible. |

Angle-specific rubrics (8–14) cover multi-hop robustness, cross-instance
generalization, null-result quality, induction consistency, harm-awareness,
overclaim guard, sense-disambiguation. Rubric 15 (scaling coherence) applies only
to multi-size sweeps — i.e. reports produced by `run_eval_matrix.py`, which
sets `is_part_of_scaling_sweep=True`. Full text: `human_review/rubrics.md`.
...
*Source of truth is always the code: `automechinterp/eval/rubrics.py` for the
rubric, `experiments/seap/FINDINGS.md` for the S-EAP benchmark, the module
