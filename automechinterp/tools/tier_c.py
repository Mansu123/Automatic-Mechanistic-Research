...
def run_synergy_eap(handle: adapter.ModelHandle, clean_prompt: str, corrupted_prompt: str,
                    io_token: str, s_token: str, layer_range: range, n_heads: int,
                    ablate_candidates: list[tuple[int, int]] | None = None,
                    ablate_mode: str = "mean", k: int = 10) -> tuple[str, list]:
    """Tool: run_synergy_eap(). Second-order EAP. For each candidate head j,
    mean-ablate it and re-measure every other head i's first-order gradient
    attribution:

        Syn(i,j) = ( grad_i(clean) - grad_i(ablate j) ) . (A_i^clean - A_i^corrupt)

    A backup pair reads phi1(i)~0, phi1(j)~0, |Syn(i,j)| large -- the shape
    single-edge run_acdc / run_eap cannot see. Returns a digest and the list
    of (score, (l_i,h_i), (l_j,h_j)) tuples sorted by |Syn|."""
    import numpy as np
    ...
    _, corr_acts = _headwise_grad_and_acts(handle, corrupted_prompt, layers, io_id, s_id, [], ablate_mode)
    base_grad, clean_acts = _headwise_grad_and_acts(handle, clean_prompt, layers, io_id, s_id, [], ablate_mode)
    ...
    def _phi1(gmap):
        ...
        for l in layers:
            for h in range(n_heads):
                n = min(gmap[l].shape[1], clean_acts[l].shape[1], corr_acts[l].shape[1])
                g = _hs(gmap, l, h, n)
                d = _hs(clean_acts, l, h, n) - _hs(corr_acts, l, h, n)
                out[(l, h)] = float((g * d).sum())
        return out

    phi1 = _phi1(base_grad)
    if ablate_candidates is None:
        ablate_candidates = [lh for lh, _ in sorted(phi1.items(), key=lambda kv: -abs(kv[1]))[:8]]

    rows = []
    for (jl, jh) in ablate_candidates:
        g_abl, _ = _headwise_grad_and_acts(handle, clean_prompt, layers, io_id, s_id,
                                           [(jl, jh)], ablate_mode)
        for l in layers:
            for h in range(n_heads):
                if (l, h) == (jl, jh):
                    continue
                n = min(base_grad[l].shape[1], g_abl[l].shape[1],
                        clean_acts[l].shape[1], corr_acts[l].shape[1])
                dg = _hs(base_grad, l, h, n) - _hs(g_abl, l, h, n)
                di = _hs(clean_acts, l, h, n) - _hs(corr_acts, l, h, n)
                rows.append((float((dg * di).sum()), (l, h), (jl, jh)))

    rows.sort(key=lambda r: -abs(r[0]))
    lines = []
    for score, (il, ih), (jl, jh) in rows[:k]:
        backup = abs(phi1[(il, ih)]) < 0.1 and abs(phi1[(jl, jh)]) < 0.1
        lines.append(f"  ablate L{jl}H{jh} -> L{il}H{ih}: Syn={score:+.3f} "
                     f"phi1=({phi1[(jl,jh)]:+.2f},{phi1[(il,ih)]:+.2f})"
                     + ("  <- backup shape" if backup else ""))
    digest = "run_synergy_eap top interactions:\n" + "\n".join(lines)
    return digest, rows
