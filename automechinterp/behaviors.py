...
def _make_task(handle: adapter.ModelHandle, behavior: str, category: str,
    ...
    def task_metric_fn() -> float:
        ...
        for prompt, io_t, s_t in eval_prompts:
            ...
            # [-1] not [0]: some tokenizers (e.g. Qwen2's, which splits digits into
            # a generic leading-space token + the digit) encode " word" as more than
            # one piece -- [0] would grab that shared space token for every answer,
            # making io_id == s_id and the whole metric silently uninformative.
...
_RELATIVE_CLAUSE_PAIRS = [
    ("author", "authors", "who lives nearby"), ("painter", "painters", "who won the prize"),
...
def coordination_behavior(handle: adapter.ModelHandle, seed: int = 0) -> dict:
    def gen(rng):
        x, y = rng.choice(_COORDINATION_PAIRS)
        clean = f"She bought both an {x} and an" if x[0] in "aeiou" else f"She bought both a {x} and a"
        corrupted = f"She bought either an {x} or an" if x[0] in "aeiou" else f"She bought either a {x} or a"
        return clean, corrupted, y, x
...
ANGLE_16_BEHAVIORS = [
    ...
    _pair_task("Morphology: -er agent noun", "Angle 16: Morphology", [
        ("A person whose job is to teach is a", "The act itself, not the person, is to", "teacher", "teach"),
        ("A person whose job is to bake is a", "The act itself, not the person, is to", "baker", "bake"),
        ("A person who plays a sport is a", "The act itself, not the person, is to", "player", "play"),
        ("A person who writes is a", "The act itself, not the person, is to", "writer", "write"),
    ]),
    _pair_task("Morphology: -ly adverb", "Angle 16: Morphology", [
        ("The adverb formed from the adjective quick is", "The adjective form, not the adverb, is", "quickly", "quick"),
        ("The adverb formed from the adjective slow is", "The adjective form, not the adverb, is", "slowly", "slow"),
        ("The adverb formed from the adjective happy is", "The adjective form, not the adverb, is", "happily", "happy"),
        ("The adverb formed from the adjective soft is", "The adjective form, not the adverb, is", "softly", "soft"),
    ]),
    _pair_task("Morphology: past participle", "Angle 16: Morphology", [
        ("The past participle of the verb eat, as in 'has ___', is", "The plain present form of the verb is", "eaten", "eat"),
        ("The past participle of the verb write, as in 'has ___', is", "The plain present form of the verb is", "written", "write"),
        ("The past participle of the verb break, as in 'has ___', is", "The plain present form of the verb is", "broken", "break"),
        ("The past participle of the verb give, as in 'has ___', is", "The plain present form of the verb is", "given", "give"),
    ]),
...
ALL_BEHAVIORS = [b for _angle in sorted(ANGLE_BEHAVIORS) for b in ANGLE_BEHAVIORS[_angle]]
