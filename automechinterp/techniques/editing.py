...
def leace(handle, layer_idx, pos_texts, neg_texts, test_prompt, pos_token, neg_token) -> dict:
    """Belrose et al. 2023 -- LEAst-squares Concept Erasure. Closed-form
    orthogonal projection P that removes all linear predictability of the
    concept from layer-l activations while changing them as little as possible
    (whitened least-squares). Applied as an inference-time hook here; the
    'edit' is the projection, not a weight change."""
    X = torch.cat([C.capture_resid(handle, layer_idx, pos_texts),
                   C.capture_resid(handle, layer_idx, neg_texts)], 0).double()
    y = torch.tensor([1.0] * len(pos_texts) + [0.0] * len(neg_texts)).double()
    mu = X.mean(0)
    Xc = X - mu
    Sigma = (Xc.T @ Xc) / len(X) + 1e-3 * torch.eye(X.shape[1]).double()
    # whitening
    evals, evecs = torch.linalg.eigh(Sigma)
    W = evecs @ torch.diag(evals.clamp_min(1e-8).rsqrt()) @ evecs.T
    Winv = evecs @ torch.diag(evals.clamp_min(1e-8).sqrt()) @ evecs.T
    # class-mean-difference direction in whitened space
    zc = (Xc @ W)
    delta = zc[y == 1].mean(0) - zc[y == 0].mean(0)
    u = delta / delta.norm().clamp_min(1e-12)
    P_white = torch.eye(X.shape[1]).double() - torch.outer(u, u)
    P = (W @ P_white @ Winv).float().to(handle.device)      # erasure in original space
    mu_f = mu.float().to(handle.device)
    ...
    with adapter.MODEL_LOCK:
        ...
        try:
            ...
            h.remove()
    # verify erasure: re-probe after projection
    from sklearn.linear_model import LogisticRegression
    from sklearn.model_selection import cross_val_score
    Xe = ((X.float() - mu_f.cpu()) @ P.T.cpu() + mu_f.cpu()).numpy()
    acc_after = float(cross_val_score(LogisticRegression(max_iter=1000), Xe, y.numpy(),
                                      cv=min(5, len(y) // 2 or 2)).mean())
    return {"layer": layer_idx, "logit_diff_before": round(base, 3),
            "logit_diff_after_erasure": round(float(lg[io] - lg[s]), 3),
            "concept_probe_acc_after_erasure": round(acc_after, 3),
            "note": "acc ~0.5 => concept linearly erased"}
