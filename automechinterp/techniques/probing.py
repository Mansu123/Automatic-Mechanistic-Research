...
def sparse_probe(handle, layer_idx, pos_texts, neg_texts, ks=(1, 3, 5, 10)) -> dict:
    ...
    from sklearn.model_selection import cross_val_score
    X, y = _xy(handle, layer_idx, pos_texts, neg_texts)
    # rank neurons by abs t-stat between classes
    p, n = X[y == 1], X[y == 0]
    tstat = np.abs(p.mean(0) - n.mean(0)) / (np.sqrt(p.var(0) / len(p) + n.var(0) / len(n)) + 1e-8)
    order = np.argsort(-tstat)
    rows = []
    for k in ks:
        idx = order[:k]
        acc = float(cross_val_score(LogisticRegression(max_iter=1000), X[:, idx], y,
                                    cv=min(5, len(y) // 2 or 2)).mean())
        rows.append((k, round(acc, 3)))
    return {"layer": layer_idx, "accuracy_by_k": rows, "top_neurons": order[:10].tolist()}
...
def mdl_probe(handle, layer_idx, pos_texts, neg_texts) -> dict:
    ...
    bits = float(n)                      # first block ~ uniform code, 1 bit/label
    prev = cuts[0]
    for cut in cuts[1:]:
        if cut <= prev or len(set(y[:prev])) < 2:
            prev = cut
            continue
...
def geometry_of_truth(handle, layer_idx, true_texts, false_texts) -> dict:
    ...
    return {"layer": layer_idx, "linear_separability_acc": round(acc, 3),
            ...
            "note": "acc ~1.0 and separated means => a linear truth direction exists here"}
        ...
        for t in texts:
            ...
            with adapter.MODEL_LOCK:
                ...
                try:
                    ...
                    h.remove()
            rows.append(store["z"][0, -1].cpu().numpy())
