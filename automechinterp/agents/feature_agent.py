...
def build_feature_agent(handle: adapter.ModelHandle, layer_idx: int, texts: list[str],
    ...
    def do_toy_sae():
        r = superposition.train_toy_sae(handle, layer_idx, texts, expansion=4, steps=300)
        state["toy_sae"] = r
    ...
    def do_geometry():
        r = feature_geometry.feature_direction_geometry(handle, layer_idx, texts, expansion=6)
        state["geometry"] = r
        return digest_dict(r)
