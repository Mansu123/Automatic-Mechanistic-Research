...
(`--target-model`). The proposal's scaling ask (mentor feedback point 3 --
"how does the finding change across model scale?") needs the *same* battery
of behaviors run against a size ladder of models and scored against the same
rubrics, so rubric 15 (`scaling_coherence`) has something to bite on.
...
calls `run_stage_d(..., is_part_of_scaling_sweep=True)` once per model.
...
MATRIX: list[EvalModel] = [
    ...
    EvalModel("EleutherAI/pythia-6.9b", "pythia", "6.9B", 6857, 32, 4096, "MHA", WORKSTATION,
              notes="largest entry; ~14GB in float32"),
    ...
    EvalModel("HuggingFaceTB/SmolLM2-1.7B-Instruct", "llama", "1.7B", 1711, 24, 2048, "GQA", LAPTOP),
...
# Named size ladders -- a within-family sweep is what rubric 15 actually wants
# (same architecture + training recipe, only scale changes).
