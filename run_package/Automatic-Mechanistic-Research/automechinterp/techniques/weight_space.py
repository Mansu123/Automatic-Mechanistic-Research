...
def parameter_svd(handle, which: str = "attn_out", max_layers: int = 6) -> dict:
    ...
    cache = getattr(handle, "_static_weight_cache", {})
    key = (which, max_layers)
    if key in cache:
        return {**cache[key], "reused_immutable_model_measurement": True}
    rows = []
    layers = np.linspace(0, handle.n_layers-1, min(max_layers, handle.n_layers), dtype=int).tolist()
    for l in layers:
        layer = handle.layers[l]
        if which == "attn_out":
            W = adapter._find_attn_out_proj(layer).weight.detach().float()
        elif which == "mlp_out":
            W = adapter._find_mlp_out_proj(layer).weight.detach().float()
        else:
    ...
    result = {"matrix": which, "per_layer_(top_sv, effective_rank, full_rank)": rows,
              "method": "exact singular values; selected layers recorded"}
    cache[key] = result
    handle._static_weight_cache = cache
    return result
