...
def patch_layer(handle, layer_idx, clean_prompt, corrupted_prompt, clean_token, corrupted_token):
    from .colab_causal import layer_effect
    r = layer_effect(handle, layer_idx, clean_prompt, corrupted_prompt, clean_token, corrupted_token)
    handle._last_layer_effect = r
    return (f"patch_layer L{layer_idx}: fraction_recovered={r['fraction_recovered']:.6f} "
            f"(clean_metric={r['clean_metric']:.6f}, corrupted_metric={r['corrupted_metric']:.6f}, "
            f"patched_metric={r['patched_metric']:.6f}; last-prompt block update; full continuation)")
...
def attn_mlp_attribution(handle: adapter.ModelHandle, layer_idx: int, prompt: str) -> str:
    ...
    attn_mod = getattr(layer, "attn", None) or getattr(layer, "self_attn", None) or getattr(layer, "attention", None)
    mlp_mod = getattr(layer, "mlp", None) or adapter._find_mlp_out_proj(layer)
