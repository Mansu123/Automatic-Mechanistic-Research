...
from ..tools.verification import evidence_gate
...
def adjudicate(claimed_heads: list[tuple[int, int]], skeptic_state: dict,
    ...
    gated, reason = evidence_gate(claimed_heads, skeptic_state.get("verification_records", {}))
    return {"verdict": gated, "reasoning": reason,
            "advisory_judge_verdict": action.get("action", "Speculative"),
            "advisory_judge_reasoning": action.get("reasoning", ""),
            "verification_records": skeptic_state.get("verification_records", {}),
            "evidence": evidence}
