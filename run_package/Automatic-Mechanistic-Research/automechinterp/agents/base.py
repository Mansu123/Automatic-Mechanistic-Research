...
class AgentLog:
    ...
    def emit(self, agent: str, message: str, gap_id: str | None = None) -> None:
        ...
        print(f"[{agent}]{tag} {message[:1200]}" + (" [full evidence persisted]" if len(message)>1200 else ""))
...
def digest_dict(d, limit: int = 600) -> str:
    ...
    try:
        ...
        s = str(d)
    return s  # Preserve full structured evidence; console output alone is shortened.
...
class Agent:
    ...
    def run(self, initial_evidence: str = "") -> list[str]:
        ...
        for _ in range(self.max_steps):
            if self.budget.global_remaining[0] <= 0:
            ...
            if tool_name == "stop":
                self.termination = "policy_stop"
                LOG.emit(self.name, f"stop ({action.get('reasoning', '')})")
                break
            if tool_name not in self.tools:
                self.termination = "unknown_tool"
            ...
            try:
                ...
                import json
                try:
                    structured = json.loads(observation) if isinstance(observation, str) else observation
                    if isinstance(structured, dict) and ("skipped" in structured or structured.get("status") in ("not_applicable", "insufficient_data")):
                        status = "skipped"
                except (ValueError, TypeError):
                    pass
                if isinstance(observation, str) and observation.lstrip().startswith("ERROR:"):
                    self._failed_tools.add(tool_name)
                    status = "error"
                ...
                self._failed_tools.add(tool_name)
                status = "error"
            self.telemetry.append({"agent": self.name, "tool": tool_name,
