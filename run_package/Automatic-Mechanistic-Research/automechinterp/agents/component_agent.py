...
def build_component_agent(handle: adapter.ModelHandle, layer_idx: int,
    ...
    def do_eap():
        ...
        state["eap_scores"] = getattr(handle, "_last_eap", [])
    ...
    def do_acdc():
        ...
        state["head_scan"] = getattr(handle, "_last_head_scan", {})
    ...
    def do_synergy_eap():
        lo, hi = max(0, layer_idx-2), min(handle.n_layers, layer_idx+3)
        digest, rows = tier_c.run_synergy_eap(handle, clean_prompt, corrupted_prompt,
                         io_token, s_token, range(lo, hi), n_heads, k=10)
        state["synergy_eap_digest"] = digest
        state["synergy_interactions"] = rows
        # Keep interaction candidates distinct from exact sufficiency/verified claims.
        state["synergy_candidate_heads"] = sorted({i for value, i, j in rows[:10] if abs(value) >= .05})
        return digest
