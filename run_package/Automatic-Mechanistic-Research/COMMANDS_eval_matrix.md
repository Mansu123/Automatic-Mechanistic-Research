...
Scoring uses the shared 8-metric / 1–10 rubric in
`automechinterp/eval/matrix_rubric.py` (see `human_review/rubrics.md`). The
`human_review/agent_layer_matrix.py` reading aid now pivots the
`review/agent_layer_tool_matrix.csv` that `run_colab.py` +
`run_review.py --export` produce, so use that flow (not this legacy per-model
one) when you need the agent×layer grids.
