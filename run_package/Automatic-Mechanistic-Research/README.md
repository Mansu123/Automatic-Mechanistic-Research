> **Current Colab release:** use `run_colab.py` and the two bundled notebooks. See [COLAB_GUIDE.md](COLAB_GUIDE.md) for the authoritative 22-model / 203-behavior workflow and shared eight-metric rubric. The commands and scoring forms below document the older research interfaces.
...
**The mental model.** `main.py` is the entrypoint; `run_eval_matrix.py` is a
thin wrapper around it for multi-model sweeps. `run_colab.py` runs the frozen
22-model / 203-behavior release; `run_review.py` exports its review items and
graphs, and `human_review/` + `llm_review/` are the two scoring arms (same
8-metric rubric, `automechinterp/eval/matrix_rubric.py`). `main.py` builds a *task* (a clean prompt, a corrupted prompt, and a metric —
...
| `python3 main.py --stage-d` | **Stage D** — write one human-readable report per (model, behavior) for scoring against the shared 8-metric rubric | `human_review/reports/*.md`, `output/eval/stage_d_*.json` |
...
| `python3 -m llm_review.run_judge --run-dir <run>` | score a `run_colab.py` run's evidence with the GLM 5.2 judge (needs `OPENROUTER_API_KEY`) | `<run>/review/judge_scores/*.json` |
| `python3 human_review/aggregate_scores.py --run-dir <run>` | combine the reviewers' CSVs for a run, report inter-reviewer agreement + health | `<run>/review/human_health.json` |
| `python3 llm_review/aggregate_scores.py --run-dir <run>` | aggregate the GLM judge's scores for a run (provisional until validated) | `<run>/review/judge_health.json` |
| `python3 human_review/agent_layer_matrix.py --run-dir <run>` | agent × layer grid per report, from `review/agent_layer_tool_matrix.csv` | `<run>/review/agent_layer_matrix.md` |
| `python3 generate_figures.py --run-dir <run>` | three publication figures rendered from that run's exported CSVs | `output/figures/figure[1-3]_*` |
...
[`COMMANDS_eval_matrix.md`](COMMANDS_eval_matrix.md) · [`human_review/README.md`](human_review/README.md) · [`llm_review/README.md`](llm_review/README.md)
...
automechinterp/
  ...
  eval/
    matrix_rubric.py                 the single shared rubric: 8 metrics, integer 1-10,
                                      identical for the human reviewers and the GLM judge
    agreement.py                     quadratic weighted kappa + the judge-validation gate
    review_export.py                 run-dir -> review items, human/judge templates, graphs
...
human_review/                        the human half of the two-arm evaluation:
                                      rubrics.md (8 metrics, 1-10), aggregate_scores.py
                                      (inter-reviewer agreement + health),
                                      agent_layer_matrix.py (agent x layer grid)
llm_review/                          the LLM half: run_judge.py (GLM 5.2 via OpenRouter),
                                      aggregate_scores.py, README.md -- same rubric,
                                      same evidence, mirrors human_review/
```
...
- **`eval/stage_d_<model>_<timestamp>.json`** — Stage D: per-report verdicts
  and applicable rubric ids; the reports themselves land in
...
- **`<run>/review/agent_layer_matrix.md`** — `human_review/agent_layer_matrix.py`:
  agent × layer grid per report, pivoted from `review/agent_layer_tool_matrix.csv`
- **`<run>/review/human_review_aggregate.csv`** — `human_review/aggregate_scores.py`:
  per-metric means + inter-reviewer agreement (health in `human_health.json`)
...
python3 main.py --stage-d              # write per-(model,behavior) reports for scoring
...
behavior battery across a **size ladder** of models (≤ 7B) — the cross-scale
question, so the `generalization` metric has a scaling comparison to bite on —
...
commit to a long sweep. For the agent×layer grids, run a `run_colab.py` sweep
plus `run_review.py --export`, then `python3 human_review/agent_layer_matrix.py
--run-dir <run>`.
